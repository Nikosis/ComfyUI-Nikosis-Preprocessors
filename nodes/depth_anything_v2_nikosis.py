...
class DepthAnythingV2Nikosis:
    ...
    def commence(self, model, images, manual_resolution=False, resolution=1024):
        ...
        orig_H, orig_W = H, W
        manual_H, manual_W = calculate_dim(H, W, resolution)
        ...
        default_res = 1024
        t_res = default_res if default_res > min(orig_H,orig_W) else min(orig_H,orig_W)
        base_res = resolution if resolution > t_res else t_res
        images = resize_to_nearest_multiple(images, base_res)
        ...
        pbar = ProgressBar(B)
        out = []
        model_instance.to(device)  # Added and removed cls.token
        ...
        model_instance.to(offload_device)  # Offload model to CPU
        del images, normalized_images, out  # Clear tensor references
